...
public export spaces : Nat -> Either Nat Nat
spaces n with (parity n)
  spaces n@(k + k) | Even {n = k} = Left $ n * n
  spaces n@(S (k + k) ) | Odd {n = k} = Right $ 2 * (k * n + k)

public export Bingo : Nat -> Type -> Type
Bingo n a with (spaces n)
  Bingo _ a | Left k = Vect k a
  Bingo _ a | Right k = (Vect k a, a)
